...
public class NearestSelector : MonoBehaviour{
   ...
   Holojam.Tools.Pairable pairable;
   void Awake(){
   ...
   void Update(){
      ...
      foreach(Transform t in contextTargets){
         float d = Vector3.Distance(t.position,pairable.center);
         if(d<range){
      ...
      if(minDist>-1){
         ...
         target.locked = true;
         //Debug
         Debug.DrawLine(target.transform.position,pairable.center,Color.yellow);
      } else ResetTarget();
